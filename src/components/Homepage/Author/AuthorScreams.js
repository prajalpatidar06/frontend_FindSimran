...
export class AuthorScreams extends Component {
    ...
    render() {
        ...
        console.log(authScreams)
        let recentScreamsMarkup = !loading ? (
          authScreams.map((scream) => <AuthScreamCard key={scream.screamId} scream={scream} handle={this.props.user.credentials.handle}/>)
        ) : (
        ...
        return (
            <div>
                <h2 className="text-center text-blue-600 text-2xl font-bold">Your Screams</h2>
                {recentScreamsMarkup}
            </div>
        )
    }
